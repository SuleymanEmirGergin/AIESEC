...
describe("saved API client", () => {
  ...
  it("X-API-KEY basligini gondermez; anahtar yalnizca sunucuda", async () => {
    const spy = mockFetch([]);
  ...
  it("kimlik basligi gondermez; kim oldugu sunucuda oturumdan eklenir", async () => {
    // Eski surumden kalan degerler taklit icin kullanilamamali.
    localStorage.setItem("volunteer_name", "Ece");
    localStorage.setItem("api_key", "ak_eski");
    ...
    expect(headers).not.toHaveProperty("X-VOLUNTEER-NAME");
