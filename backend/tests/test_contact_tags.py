...
from app.routers.export import build_csv, format_phone
...
class TestExportCsv:
    def _rows(self, items):
        return list(csv.reader(io.StringIO(build_csv(items)), delimiter=";"))

    def test_email_column_exists(self):
        header = self._rows([])[0]
        assert "E-posta" in header
        assert header.index("E-posta") == header.index("Telefon") + 1
    ...
    def test_prefers_plain_tag_over_prefixed(self):
        ...
        assert row[header.index("Telefon")] == "+90 212 111 11 11"
    ...
    def test_falls_back_to_prefixed_scheme(self):
        ...
        header, row = rows[0], rows[1]
        assert row[header.index("Telefon")] == "+90 212 333 33 33"
        assert row[header.index("E-posta")] == "bilgi@ornek.com"
        assert row[header.index("Web Sitesi")] == "https://ornek.com"
    ...
    def test_missing_tags_produce_empty_cells_not_crash(self):
        ...
        header, row = rows[0], rows[1]
        assert row[header.index("Telefon")] == ""
        assert row[header.index("E-posta")] == ""

    def test_location_is_single_text_cell_with_map_link(self):
        rows = self._rows(
            [{"id": "osm:node:4", "name": "Konumlu", "type": "office", "lat": 41.0307426, "lon": 28.8605461}]
        )
        header, row = rows[0], rows[1]
        assert row[header.index("Konum")] == "41.030743, 28.860546"
        assert row[header.index("Harita")] == "https://www.google.com/maps?q=41.030743,28.860546"
        assert row[header.index("Tür")] == "Ofis"

    def test_placeholder_address_is_dropped(self):
        rows = self._rows(
            [{"id": "osm:node:5", "name": "Adressiz", "type": "office", "lat": 41.0, "lon": 29.0, "address": "Adres bilgisi yok"}]
        )
        header, row = rows[0], rows[1]
        assert row[header.index("Adres")] == ""


class TestFormatPhone:
    def test_turkish_numbers_get_readable_spacing(self):
        assert format_phone("+905424703486") == "+90 542 470 34 86"
        assert format_phone("05424703486") == "+90 542 470 34 86"
        assert format_phone("(0212) 442 00 58") == "+90 212 442 00 58"

    def test_already_spaced_is_preserved(self):
        assert format_phone("+90 212 111 11 11") == "+90 212 111 11 11"

    def test_unknown_shape_passes_through(self):
        assert format_phone("0555 188 39 15 dahili 3") == "0555 188 39 15 dahili 3"
        assert format_phone("") == ""
