...
async def verify_everything():
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        free_resp = await client.post(
            f"{BASE_URL}/admin/keys?name=tester_free&plan=free",
        ...
        pro_resp = await client.post(
            f"{BASE_URL}/admin/keys?name=tester_pro&plan=pro",
            headers={"X-ADMIN-KEY": ADMIN_KEY},
        )
        ...
        r_free = await client.get(
            f"{BASE_URL}/api/search?type=factory&lat=41.0&lon=28.9&radius=3000",
        ...
        print(
            f"Pro Key (3000m): Status {r_pro.status_code} (Expected 200 / 429 if Overpass busy)"
        )

        print("\n--- Phase 3: Verifying Plan Gating (Export) ---")
        # 3.1 Free key export -> Expect 403
        e_free = await client.post(
        ...
        if r_pro.status_code == 200:
            ...
            if search_res["results"]:
                ...
                print(
                    f"Result Intelligence: confidence={p.get('confidence')}, level={p.get('confidence_level')}"
                )
                if "confidence" in p:
            ...
            print(
                f"NOTE: Pro search returned {r_pro.status_code}, skipping score check."
            )

        print("\n--- Phase 5: Verifying Grid Caching ---")
        # Using coordinates strictly within the same 0.01 grid bucket
        t1 = await client.get(
            f"{BASE_URL}/api/search?type=factory&lat=41.011&lon=28.911&radius=1000",
            headers={"X-API-KEY": pro_key},
        )
        t2 = await client.get(
            f"{BASE_URL}/api/search?type=factory&lat=41.014&lon=28.914&radius=1000",
            headers={"X-API-KEY": pro_key},
        )
        ...
        print(
            f"Grid Cache Test: Status 1: {t1.status_code}, Status 2: {t2.status_code}"
        )
